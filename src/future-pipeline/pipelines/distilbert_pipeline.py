...
from future_pipeline import FuturePipeline


class DistilBertPipeline(FuturePipeline):
    ...
    def filter(self, prediction, *args):
        return prediction.item() > self.prediction_threshold


if __name__ == "__main__":

    out_file = "test_new_pipe_{}.jsonl".format(datetime.now().isoformat())
    p = DistilBertPipeline(
        out_file=out_file,
